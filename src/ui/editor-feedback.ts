...
export function installEditorFeedback(): void {
  ...
  document.addEventListener('click', event => {
    ...
    const navigation = button?.matches('[data-workspace-tab], [data-module-index], [data-module], [data-menu-card], #btnBack, #btnSair, [id*="Cancel"], [id^="cancel"], [id^="close"], #serviceSchedule, #serviceConfig, #servicePrev, #serviceNext, [data-edit-service-template]') || target.classList.contains('modal-overlay') || Boolean(button && [...active(pending), ...active(busy)].some(scope => !scope.contains(button)))
