...
export interface Talk {
  ...
  oradorId?: string; oradorNome?: string; oradorSecundarioId?: string; oradorSecundarioNome?: string
...
export function talkConflicts(talkId: string, talk: Talk, talks: Record<string, Talk>, taskMeetings: { date?: string; assignments?: Record<string, unknown> }[], speakers: Record<string, Speaker>): string[] {
  ...
  const ids = [talk.oradorId, talk.oradorSecundarioId].filter((id): id is string => Boolean(id))
  ...
  for (const other of Object.values(talks)) {
    ...
    const otherIds = [other.oradorId, other.oradorSecundarioId]
