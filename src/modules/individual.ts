...
function render(): void {
  ...
  root.innerHTML = `${moduleTitle('Minha agenda')}${screenTabs()}${loadingAssignments ? '<div class="notice">Atualizando designações dos módulos...</div>' : ''}${adminPersonPicker()}
    ...
    <details class="form-panel agenda-board-card agenda-personal-panel"><summary><strong>Mais opções</strong><span>Calendário, compartilhamento e histórico</span></summary><div class="agenda-board-body">${calendarExportPanel()}${historyPanel()}</div></details>`
  bind()
}
