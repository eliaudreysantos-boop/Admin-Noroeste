...
function renderReportScreen(root: HTMLElement): void {
  const masterId = ctx!.usuario.masterId!, secretary = (data.secretario ?? {}) as Record<string, unknown>, publisher = Object.values((secretary['publicadores'] ?? {}) as Record<string, SecretaryPublisher>).find(item => item.masterId === masterId && item.ativo)
  ...
  const reportAction = publisher ? '<button class="btn btn-primary" id="openPersonalReport" type="button">Enviar relatório</button>' : '<div class="notice warning">Seu cadastro ainda não foi vinculado como publicador pelo secretário.</div>'
  root.innerHTML = `${moduleTitle('Minha agenda')}${screenTabs()}<div class="program-summary"><div><strong>${serviceReports.length}</strong><span>Relatórios no ano</span></div><div><strong>${studies}</strong><span>Estudos bíblicos</span></div>${pioneerProgress}<div><strong>${year}/${String(year + 1).slice(-2)}</strong><span>Ano de serviço</span></div></div><div class="form-panel"><h3 style="margin-top:0">Relatório de serviço</h3><p class="form-help">${publisher ? `${esc(CATEGORY_LABELS[publisher.categoria])}. Envie seu relatório mensal e acompanhe os registros já recebidos.` : 'Aguarde o vínculo do seu cadastro pelo secretário para enviar o relatório.'}</p>${reportAction}</div><div class="module-option-list">${serviceReports.sort((a, b) => String(b.competencia).localeCompare(String(a.competencia))).map(report => `<div class="agenda-event"><time>${esc(String(report.competencia))}</time><div><strong>${report.participou === false ? 'Não participou' : 'Participou no ministério'}</strong><small>${Number(report.estudos) || 0} estudo(s) bíblico(s)${report.horasCampo ? ` · ${report.horasCampo} hora(s)` : ''}</small></div></div>`).join('') || '<p class="empty-state">Nenhum relatório neste ano de serviço.</p>'}</div>`
...
function openReport(): void {
  ...
  const secretary = (data.secretario ?? {}) as Record<string, unknown>, publisher = Object.values((secretary['publicadores'] ?? {}) as Record<string, SecretaryPublisher>).find(item => item.masterId === ctx!.usuario.masterId && item.ativo)
  ...
  document.getElementById('personalReport')?.addEventListener('submit', async event => {
    ...
    const reports = ((data.secretario ?? {}) as Record<string, unknown>)['relatorios'] as Record<string, SecretaryReport> ?? {}, current = Object.values(reports).find(report => report.masterId === ctx!.usuario.masterId && report.competencia === competence)
    if (current?.origem === 'secretario') { alert('Este relatório já foi lançado pelo secretário.'); return }
    const now = new Date().toISOString(), id = current?.id ?? `pessoal-${ctx!.usuario.masterId}-${competence}`, item = normalizePersonalReport({ id, masterId:ctx!.usuario.masterId!, competencia:competence, categoria:publisher.categoria as PublisherCategory, participou:values.get('participou') === 'on', estudos:values.get('estudos'), horasCampo:values.get('horasCampo'), observacoes:values.get('observacoes'), recebidoEm:current?.recebidoEm ?? now.slice(0, 10), atualizadoEm:now })
