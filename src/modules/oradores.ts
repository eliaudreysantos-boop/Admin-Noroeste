...
function programActionsOpen(id: string): boolean {
  const saved = localStorage.getItem(`noroeste:oradores:actions:${id}`)
  return saved === null ? window.matchMedia('(min-width: 700px)').matches : saved === 'true'
}
...
function render(): void {
  ...
  el.querySelectorAll<HTMLDetailsElement>('[data-program-options]').forEach(panel => panel.addEventListener('toggle', () => {
    try { localStorage.setItem(`noroeste:oradores:actions:${panel.dataset['programOptions']}`, String(panel.open)) } catch { /* A preferencia nao impede as acoes. */ }
  }))
...
function openExchangeDraft(): void {
  ...
  const message = `${congregation.contato ? `Olá, ${congregation.contato}.` : 'Olá.'} Segue nosso acompanhamento de intercâmbios:\n${entries.map(([, talk]) => `- ${formatDate(talk.data)} · ${canonicalTalkType(talk.tipo) === 'saida_orador' ? 'Arranjo Externo' : 'Entrada'} · ${talk.oradorNome ?? oradorNome(talk.oradorId ?? '', discursos.oradores?.[talk.oradorId ?? ''])} · ${deriveStatus(talk) === 'confirmado' ? 'confirmado' : 'a confirmar'}`).join('\n') || '- Nenhum intercâmbio futuro'}`
