...
function renderAvailableDates(horizon=90):void {
  ...
  panel.innerHTML=`<label class="form-field"><span>Próximos dias</span><select id="availableHorizon">${[90,180,365].map(value=>option(String(value),`${value} dias`,String(horizon))).join('')}</select></label>${contextSelect('congregation')}<p class="form-help">${esc(local?.nome||'Cadastre a congregação local e configure o dia da reunião.')}. Somente datas sem programação local; saídas não ocupam a reunião.</p><div style="max-height:240px;overflow:auto">${dates.map(date=>`<label style="display:block"><input type="checkbox" data-available-date="${date}" checked> ${esc(formatSpeakerDate(date))} · ${esc(planning.s2Time||local?.horario||'Horário a confirmar')}</label>`).join('')||empty('Nenhuma data disponível neste intervalo.')}</div><button id="sendAvailableDates" class="btn btn-primary" ${dates.length&&selectedCongregationId?'':'disabled'}>Abrir datas selecionadas no WhatsApp</button>`
  ...
  document.getElementById('congregationContext')?.addEventListener('change',event=>{selectedCongregationId=(event.target as HTMLSelectElement).value;renderAvailableDates(horizon)})
  document.getElementById('sendAvailableDates')?.addEventListener('click',()=>{
    ...
    openWhatsapp(congregations()[selectedCongregationId]?.telefone||'',availableMessage(data,selectedCongregationId,selected,planning.s2Time,messageSettings.meetingText))
  })
}
function renderAssignments():void {
  const speaker=speakers()[selectedSpeakerId],entries=assignmentEntries(data,selectedSpeakerId,today(),planning.s2Time)
  root().innerHTML=`${sectionTitle('Designações')}${contextSelect('speaker')}${speaker&&entries.length?'<button id="notifySpeakerAssignments" class="btn btn-primary">Abrir no WhatsApp</button>':''}<div class="oradores-list">${speaker?entries.map(entry=>'<article class="oradores-card" style="white-space:pre-wrap">'+esc(entry.text)+'</article>').join('')||empty(speaker.nome+' não tem compromissos futuros.'):empty('Nenhum orador selecionado.')}</div>`
