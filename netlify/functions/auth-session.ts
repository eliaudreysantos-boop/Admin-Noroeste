import { appSession, clearAppCookie, clearLoginFailures, createAppSession, destroyAppSession, json, loginAttemptAllowed, objectBody, recordLoginFailure, verifyPassword } from '../lib/secure-session.ts'

export default async (request: Request): Promise<Response> => {
  if (request.method === 'GET') {
    try {
      const session = await appSession(request)
      return session ? json(200, { uid:session.uid, usuario:session.usuario, csrf:session.csrf }) : json(200, { authenticated:false }, { 'set-cookie':clearAppCookie() })
  ...
  if (request.method === 'POST') {
    ...
    try {
      ...
      return json(200, { uid:authenticated.uid, usuario:authenticated.usuario, csrf:created.session.csrf }, { 'set-cookie':created.cookie })
